...
class Robot_Nav_Publisher : public rclcpp::Node{
    public:
        Robot_Nav_Publisher(): Node("Robot_Control"){
            subscriber_ = this->create_subscription<nav_msgs::msg::Odometry>(
                "/x3/odometry",
                10,
                std::bind(&Robot_Nav_Publisher::PosCallback, this, std::placeholders::_1)
            );
            publisher_ = this->create_publisher<geometry_msgs::msg::Twist>("forward_velocity_controller/commands", 10);
...
int main(int argc, char * argv[]){
    ...
    rclcpp::spin(std::make_shared<Robot_Nav_Publisher>());
    rclcpp::shutdown();
    return 0;
}